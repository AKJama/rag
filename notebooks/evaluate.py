"""
NDCG@10 across BM25, dense, hybrid (RRF), and hybrid + rerank,
...
paraphrase-heavy questions, and every stage of the pipeline improves the retrieval metric.
...
RATE_LIMIT_INTERVAL = 60.0 / RATE_LIMIT_CALLS_PER_MINUTE
_last_call_time = 0.0
...
def hybrid_topk(query: str, candidate_k: int = 50) -> list[str]:
    return [d for d, _ in hybrid_candidates(query, bm25, dense, candidate_k=candidate_k)]
...
def reranked_topk(query: str, candidate_k: int = 50, k: int = 10) -> list[str]:
    ...
    for attempt in range(max_retries):
        try:
            result = rerank_with_cohere(query, candidates, corpus_by_id, k=k)
