...
A bi-encoder embeds the query and the document separately, then compares with cosine.
A cross-encoder feeds the query and document INTO the same model and returns a single relevance score.
Cross-encoders are slower, but the joint attention catches subtleties that
two independent embeddings miss.

I'll use Cohere rerank-v4.0-fast.
